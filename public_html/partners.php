...
<head>
    ...
    // 後方互換: categoryが未指定でtypeが指定されている場合はcategoryにマッピング
    if ($selectedCategory === '' && isset($_GET['type']) && $_GET['type'] !== '') {
    ...
    function build_partners_url(int $toPage, array $keep = ['area', 'category']): string
    {
        $qs = [];
        foreach ($keep as $k) {
...
<?php
    ...
    foreach ($categoryOptions as $value => $label):
        $qs = [];
        if ($value !== '') { $qs['category'] = $value; }
        if ($selectedArea !== '') { $qs['area'] = $selectedArea; }
        $href = '/partners/' . (!empty($qs) ? ('?' . http_build_query($qs)) : '');
        $isActive = ($value === '' && ($selectedCategory === '' || $selectedCategory === 'all')) || $selectedCategory === $value;
?>
                                    <a href="<?php echo h($href); ?>" class="filter-tag <?php echo $isActive ? 'active' : ''; ?>"><?php echo h($label); ?></a>
<?php endforeach; ?>
                                </div>
