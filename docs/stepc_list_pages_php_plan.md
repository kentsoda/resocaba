...
## 完了基準
- [x] `jobs.php` が DB ループ + ページネーションで描画 ✅
- [x] `features.php` が DB ループ + ページネーションで描画 ✅
- [x] `announcements.php` が DB ループ + ページネーションで描画 ✅
- [x] `partners.php` が DB ループ + ページネーションで描画 ✅
- [x] `faq.php` が DB ループで描画 ✅
- [x] すべてのリンクが StepB 規約（拡張子なし/末尾スラッシュ）に準拠 ✅
- [x] UI/UX の変更なし（見た目・クラス・構造を維持） ✅
